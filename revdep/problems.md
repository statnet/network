...
*   checking whether package ‘CEMiTool’ can be installed ... WARNING
    ...
    See ‘/srv/scratch/z3528859/github/statnet/network/revdep/checks/CEMiTool/new/CEMiTool.Rcheck/00install.out’ for details.
    ```
...
*   checking examples ... ERROR
    ...
    The following values are affected:
    	...
    	- `x[10, 1:2]`
    	- `x[15, 1:2]`
    Warning: In a call to `as.network()` for `x` a `data.frame`, `loops` is `FALSE`, but `x` contains self-loops. In the future, this will be an error.
    Error in add.edges.network(x = out, tail = out_sources, head = out_targets,  : 
      formal argument "names.eval" matched by multiple actual arguments
    Calls: plot1Att -> <Anonymous> -> as.network.data.frame
    Execution halted
    ```
...
*   checking examples ... ERROR
    ...
    > ### Name: mod_ora
    > ### Title: # Run module overrepresentation analysis
...
*   checking tests ...
    ...
    Last 13 lines of output:
      ...
      Round 13 
      [1] "first_prime  round ( |s_prime| = 14 )"
         0.03296036   0.00940938   Removed feature  162 
      ══ Failed tests ════════════════════════════════════════════════════════════════
      ── Error (test-fcoex.R:12:1): (code run outside of `test_that()`) ──────────────
      Error: invalid 'nchars' argument
      Backtrace:
          █
       1. └─pathwayPCA::read_gmt(gmt) test-fcoex.R:12:0
       2.   └─base::readChar(file, nchars = nChars, useBytes = TRUE)
...
*   checking tests ...
    ...
    Last 13 lines of output:
           █
        1. ├─testthat::expect_error(...) test-ggnetworkmap.R:208:2
        2. │ └─testthat:::quasi_capture(...)
        3. │   ├─testthat:::.capture(...)
        4. │   │ └─base::withCallingHandlers(...)
        5. │   └─rlang::eval_bare(quo_get_expr(.quo), quo_get_env(.quo))
        6. ├─GGally::ggnetworkmap(net = network(data.frame(1:2, 3:4), hyper = TRUE))
        7. └─network::network(data.frame(1:2, 3:4), hyper = TRUE)
        8.   ├─network::as.network(...)
        9.   └─network::as.network.data.frame(...)
       10.     └─network:::.validate_edge_df(...)
      
      [ FAIL 3 | WARN 1330 | SKIP 3 | PASS 612 ]
      Error: Test failures
      Execution halted
    ```
