...
check[53]<-all(as.sociomatrix(temp,"value")==(g*matrix(1:100,10,10))[g>0])
...
check[60]<-all(((g%*%g)>0)==as.sociomatrix("%*%.network"(temp,temp2)))
check[61]<-all(g%*%(2*g)==as.sociomatrix("%*%.network"(temp,temp2,"value"),"value"))
check[62]<-all(((g%*%g)>0)==as.sociomatrix(temp%c%temp2))

#"Stress tests" -- try to make the code segfault!
temp<-network.initialize(-1)
temp<-add.edge(temp,4,-5)
