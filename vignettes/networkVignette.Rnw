\documentclass[article,shortnames]{jssmod}
...
\begin{description}
\item[\code{bipartite}] This is a \code{numeric} attribute, which is used to indicate the presence of an
...
edges represent connections between two distinct classes of entities.)  Where \code{bipartite > 0},
...
\item[\code{n}] Finally, \code{n} is a \code{numeric} attribute containing the number of elements in the vertex set.  Applicable methods are expected to adjust this attribute up or down, should vertices be added or deleted from the network.
\end{description}
...
networks \citep[useful for joint representation of cognitive and behavioral structures such as those of][]{krackhardt:sn:1988,killworth.bernard:ho:1976}, edge timing information (for dynamic structures), etc.  Appropriate use of network, edge, and vertex attributes allows a wide range of complex relational data structures to be supported without the need for a cumbersome array of of custom data classes.
